...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_btn'])) {
    ...
    if (empty($name) || empty($email) || empty($username) || empty($student_id)) {
        ...
        if (mysqli_num_rows($check_email_query_run) > 0) {
            ...
            $sql = "UPDATE users SET name=?, email=?, username=?, student_id=? WHERE id=?";
            $stmt = $con->prepare($sql);
            $stmt->bind_param("ssssi", $name, $email, $username, $student_id, $_SESSION['user_id']);
            $stmt->execute();

            if ($query_run) {
                sendemail_verify($name, $email, $username, $student_id);
...
<div class="container d-flex justify-content-center align-items-center vh-100" 
    ...
    <div class="card border-0" >
        ...
        <div class="card-body rounded-bottom bg-white p-5"  style="width: 600px;" >
            ...
            <div>
                <button class="btn btn-primary btn-block py-3" name="submit_btn" type="submit">Submit</button>
            </div>
