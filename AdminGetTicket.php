                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="mb-3 mb-md-0">
                                    <select class="custom-select px-4" name="gateSelect" id="gateSelect" style="height: 47px;" required>
                            ...
                            <div class="col-md-3">
                                <div class="mb-3 mb-md-0">
                                    <input type="text" class="form-control p-4 datetimepicker-input" name="studentID" id="studentIDInput" placeholder="Student ID" required/>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mb-3 mb-md-0">
                                    <input type="text" class="form-control p-4 datetimepicker-input" name="vehicleNo" id="vehicleNoInput" placeholder="Vehicle No." required/>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mb-3 mb-md-0">
                                    <input type="text" class="form-control p-4 datetimepicker-input" name="amount" id="amount" placeholder="Amount" required/>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <button class="btn btn-primary btn-block" type="submit" style="height: 47px; margin-top: -2px;">Get Ticket</button>
                    </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['gateSelect']) && isset($_POST['studentID']) && isset($_POST['vehicleNo']) && isset($_POST['amount'])) {
    ...
    if (!is_dir($qrDirectory)) {
        mkdir($qrDirectory, 0777, true); // The last parameter "true" allows the creation of nested directories as needed
    }
    ...
    $database = "park"; // Change this to your database name
    ...
    // Fetch form data
    $gateNumber = $_POST['gateSelect'];
    $studentID = $_POST['studentID'];
    $vehicleNumber = $_POST['vehicleNo'];
    $amount = $_POST['amount'];

    // Retrieve username and email based on student ID
    ...
    // SQL query to insert data into transaction table
    $sql = "INSERT INTO transaction (gate_number, student_id, username, vehicle_no, amount, transaction_type, email) VALUES (?, ?, ?, ?, ?, 'Ticket', ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssis", $gateNumber, $studentID, $username, $vehicleNumber, $amount, $email);

    // Execute the transaction insertion query
    if ($stmt->execute()) {
        // SQL query to insert data into tickets table
        $sql2 = "INSERT INTO tickets (gate_number, student_id, vehicle_number, amount) VALUES (?, ?, ?, ?)";
        $stmt2 = $conn->prepare($sql2);
        $stmt2->bind_param("ssss", $gateNumber, $studentID, $vehicleNumber, $amount);

        // Execute the tickets insertion query
        if ($stmt2->execute()) {
            // Ticket and QR code generation

            // Generate ticket HTML content
            $ticket = "<div class='contact-form bg-white' style='padding: 30px; border: 2px solid #000; text-align: center; border-radius: 10px;'>"
                . "<h4>AGC P-Ticket</h4>"
                . "<hr>"
                . "<p>Date: " . date("d-m-Y") . "<br>" // Add current date
                . "Time: " . date("H:i:s") . "</p>" // Add current time
                . "<p>Gate Number: " . $gateNumber . "<br>"
                . "Student ID: " . $studentID . "<br>"
                . "Vehicle Number: " . $vehicleNumber . "<br>"
                . "Amount: " . $amount . " /-</p>";

            // Generate QR code
            $qrText = "Gate Number: " . $gateNumber . "\n"
                . "Student ID: " . $studentID . "\n"
                . "Vehicle Number: " . $vehicleNumber . "\n"
                . "Amount: " . $amount;
            $qrFile = $qrDirectory . 'ticket_qr.png'; // Path to save the QR code image
            QRcode::png($qrText, $qrFile, QR_ECLEVEL_L, 4);

            // Output ticket and QR code
            echo $ticket;
            echo '<img src="' . $qrFile . '" alt="QR Code"/><br>';
            echo '<button onclick="window.print()" class="btn btn-primary">Print Ticket</button>';
            echo '</div>';
        } else {
            // Error in inserting data into tickets table
        ...
        // Error in inserting data into transaction table
