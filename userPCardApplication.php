...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['apply_btn'])) {
    ...
    if (empty($name) || empty($username) || empty($email) || empty($rollno) || empty($course) || empty($student_id) || empty($mobile) || empty($vehicle_number) || empty($address)) {
        echo "<p class='text-danger'>All fields are required</p>";
    } else {
        // Prepare and execute SQL statement
        $stmt = $conn->prepare("INSERT INTO pcard (name, username, email, rollno, course, student_id, mobile, vehicle_number, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssss", $name, $username, $email, $rollno, $course, $student_id, $mobile, $vehicle_number, $address);

        if ($stmt->execute()) {
            // Send email notification
            sendEmailVerification($name, $username, $email, $rollno, $course, $student_id, $mobile, $vehicle_number, $address);

            // Display success message
            echo "<script>alert('P-Card Application Form Submitted successfully.');</script>";
            echo "<script>window.location.href = 'userHome.php';</script>";
            exit; // Prevent further execution
        } else {
            // Display error message
            echo "<p class='text-danger'>Error: " . $stmt->error . "</p>";
            echo "<script>window.location.href = 'userPCardApplication.php';</script>";
        }
...
function sendEmailVerification($name, $username, $email, $rollno, $course, $student_id, $mobile, $vehicle_number, $address)
{
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Parking Card Application Submission Confirmation';
        $mail->Body = "<h3>Dear $name,</h3>
        
        <h4>We're pleased to inform you that your parking card application has been successfully submitted.<br><br>
        
        You can view the status of your parking card application by logging into your account. 
        Once logged in, navigate to your account dashboard where you'll find information regarding your parking card.<br>
        
        If you have any questions or need further assistance, please don't hesitate to contact us at iparksmart45@gmail.com or +91 2041038200. We're here to help!<br><br>
        
        Thank you for choosing I PARK SMART.<br><br><br>
                    ...
                    <div class="navbar-nav ml-auto py-0">
                        ...
                        <div class="nav-item dropdown">
                            ...
                            <a class="dropdown-item" href="userProfile.php"><?php echo $_SESSION["username"]; ?></a>
                                ...
                                <a class="dropdown-item active" href="userPCardApplication.php">P-Card Application</a>
        ...
        <div class="container">
            <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 400px">
                ...
                <div class="d-inline-flex text-white">
                    ...
                    <p class="m-0 text-uppercase">P-Card Application</p>
                </div>
            </div>
        </div>
    </div>
...
<!-- P-Card Application Start -->
    <div class="container-fluid booking mt-5 pb-5">
        <div class="container pb-5">
            <div class="bg-light shadow" style="padding: 30px;">
                <h6 class="text-primary text-uppercase" style="letter-spacing: 5px; font-size: 20px;">Card Application</h6><hr>
                <form action="#" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="<?php echo $_SESSION["username"];?> "class="form-control" name="username" id="username" readonly>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="rollno" class="required-label">Roll No. </label>
                        <input type="text" class="form-control" name="rollno" id="rollno" required="required">
                    </div>
                    <div class="form-group">
                        <label for="course" class="required-label">Course</label>
                        <input type="text" class="form-control" name="course" id="course" required="required">
                    </div>
                    <div class="form-group">
                        <label for="student_id" class="required-label">Student ID</label>
                        <input type="text" class="form-control" name="student_id" id="student_id" required="required">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="vehicle_number" class="required-label">Vehicle Number </label>
                        <input type="text" class="form-control" name="vehicle_number" id="vehicle_number" placeholder="last 4 digit">
                    </div>
                    <div class="form-group">
                        <label for="address" class="required-label">Address</label>
                        <input type="text" class="form-control" name="address" id="address" required="required" placeholder="Area and City">
                    </div>
                    <div class="form-group mb-0">
                        <input type="submit" value="Apply" name="apply_btn" class="btn btn-primary font-weight-semi-bold py-2 px-3">
                    </div>
                </form>                  
            </div>   
        </div>
    </div>
    <!-- P-Card Application End -->
